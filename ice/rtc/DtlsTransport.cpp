...
#define MS_ASSERT(con, ...) do { if(!(con)) { hloge(__VA_ARGS__); std::runtime_error("MS_ASSERT"); } } while(false)
#define MS_ABORT(...) do { hloge(__VA_ARGS__); abort(); } while(false)
...
namespace RTC
{
    ...
    DtlsTransport::DtlsEnvironment& DtlsTransport::DtlsEnvironment::Instance() {
        static std::shared_ptr<DtlsTransport::DtlsEnvironment> instance(new DtlsTransport::DtlsEnvironment());
        return *instance;
    }
    ...
    DtlsTransport::DtlsEnvironment::DtlsEnvironment()
    {
        ...
        SSL_CTX* ssl = (SSL_CTX*)hssl_ctx_instance();
        if (!ssl || !ReadCertificateAndPrivateKeyFromContext(ssl)) {
    ...
    bool DtlsTransport::DtlsEnvironment::ReadCertificateAndPrivateKeyFromContext(SSL_CTX *ctx)
    {
        ...
        hlogi("Load webrtc dtls certificate");
        return true;
    }
    ...
    void DtlsTransport::DtlsEnvironment::CreateSslCtx()
    {
        ...
        hlogi("setting SRTP cryptoSuites for DTLS: %s", dtlsSrtpCryptoSuites.c_str());
        ...
        if (ret != 0)
        {
            MS_ERROR("SSL_CTX_set_tlsext_use_srtp() failed when entering %s", dtlsSrtpCryptoSuites.c_str());
    ...
    void DtlsTransport::DtlsEnvironment::GenerateFingerprints()
    {
        ...
        for (auto& kv : DtlsTransport::string2FingerprintAlgorithm)
        {
            ...
            hlogd("%s fingerprint=%s", algorithmString.c_str(), hexFingerprint);
    ...
    DtlsTransport::DtlsTransport(hv::EventLoopPtr poller,Listener* listener) : poller(poller), listener(listener)
    {
        MS_TRACE();
    ...
    void DtlsTransport::Dump() const
    {
        ...
        hlogi("DtlsTransport %s: state=%s, role=%s, handshake done=%s", getId(), state.c_str(), role.c_str(), this->handshakeDone ? "yes" : "no");
    }

    void DtlsTransport::Run(Role localRole)
    {
        hlogi("DtlsTransport %s: running as %s", getId(), (localRole == RTC::DtlsTransport::Role::SERVER)? "Server" : "Client");
        ...
        {
            hlogw("DtlsTransport %s: same local DTLS role provided, doing nothing", getId());
        ...
        {
            hlogd("DtlsTransport %s: resetting DTLS due to local role change", getId());
            Reset();
        }
        ...
        switch (this->localRole)
        {
            ...
            case Role::SERVER:
            {
                hlogd("DtlsTransport %s: running [role:server]", getId());
    ...
    bool DtlsTransport::SetRemoteFingerprint(Fingerprint fingerprint)
    {
        ...
        {
            hlogd("DtlsTransport %s: handshake already done, processing it right now", getId());
    ...
    void DtlsTransport::ProcessDtlsData(const uint8_t* data, size_t len)
    {
        ...
        if (!IsRunning())
        {
            hlogw("DtlsTransport %s: cannot process data while not running", getId());
            return;
        }
        ...
        {
            hlogw("DtlsTransport %s: OpenSSL BIO_write() wrote less (%d bytes) than given data (%zu bytes)", getId(), written, len);
        }
        ...
        if (read > 0)
        {
            ...
            {
                hlogw("DtlsTransport %s: ignoring application data received while DTLS handshake not done", getId());
                return;
            }
    ...
    void DtlsTransport::SendApplicationData(const uint8_t* data, size_t len)
    {
        ...
        {
            hlogw("DtlsTransport %s: cannot send application data while DTLS is not fully connected", getId());
            return;
        }

        if (len == 0)
        {
            hlogw("DtlsTransport %s: ignoring 0 length data", getId());
            return;
        }
        ...
        {
            hlogw("DtlsTransport %s: OpenSSL SSL_write() wrote less (%d bytes) than given data (%zu bytes)", getId(), written, len);
        }
    ...
    void DtlsTransport::Reset()
    {
        ...
        hlogw("DtlsTransport %s: resetting DTLS transport", getId());
    ...
    inline bool DtlsTransport::CheckStatus(int returnCode)
    {
        ...
        switch (err)
        {
            ...
            case SSL_ERROR_WANT_WRITE:
                hlogw("DtlsTransport %s: SSL status: SSL_ERROR_WANT_WRITE", getId());
                break;

            case SSL_ERROR_WANT_X509_LOOKUP:
                hlogd("DtlsTransport %s: SSL status: SSL_ERROR_WANT_X509_LOOKUP", getId());
                break;
            ...
            case SSL_ERROR_WANT_CONNECT:
                hlogw("DtlsTransport %s: SSL status: SSL_ERROR_WANT_CONNECT", getId());
                break;

            case SSL_ERROR_WANT_ACCEPT:
                hlogw("DtlsTransport %s: SSL status: SSL_ERROR_WANT_ACCEPT", getId());
                break;

            default:
                hlogw("DtlsTransport %s: SSL status: unknown error", getId());
        }
        ...
        else if (((SSL_get_shutdown(this->ssl) & SSL_RECEIVED_SHUTDOWN) != 0) || err == SSL_ERROR_SSL || err == SSL_ERROR_SYSCALL)
        {
            if (this->state == DtlsState::CONNECTED)
            {
                hlogd("DtlsTransport %s: disconnected", getId());
                Reset();
            ...
            {
                hlogw("DtlsTransport %s: connection failed", getId());
                Reset();
    ...
    inline void DtlsTransport::SendPendingOutgoingDtlsData()
    {
        ...
        hlogd("DtlsTransport %s: %" PRId64 " bytes of DTLS data ready to sent to the peer", getId(), read);
    ...
    inline bool DtlsTransport::SetTimeout()
    {
        ...
        {
            hlogw("DtlsTransport %s: DTLS timeout too high (%" PRIu64 "ms), resetting DTLS", getId(), timeoutMs);
    ...
    inline bool DtlsTransport::ProcessHandshake()
    {
        ...
        hlogw("DtlsTransport %s: SRTP crypto suite not negotiated", getId());
    ...
    inline bool DtlsTransport::CheckRemoteFingerprint()
    {
        ...
        if (!certificate)
        {
            hlogw("DtlsTransport %s: no certificate was provided by the peer", getId());
            return false;
        }
        ...
        {
            hlogw("DtlsTransport %s: fingerprint in the remote certificate (%s) does not match the announced one: %s", getId(), hexFingerprint, this->remoteFingerprint.value.c_str());
            X509_free(certificate);
            return false;
        }

        hlogd("DtlsTransport %s: valid remote fingerprint", getId());
    ...
    inline RTC::SrtpSession::CryptoSuite DtlsTransport::GetNegotiatedSrtpCryptoSuite()
    {
        ...
        for (auto& srtpCryptoSuite : DtlsTransport::srtpCryptoSuites)
        {
            ...
            {
                hlogd("DtlsTransport %s: chosen SRTP crypto suite: %s", getId(), cryptoSuiteEntry->name);
    ...
    inline void DtlsTransport::OnSslInfo(int where, int ret)
    {
        ...
        {
            hlogd("DtlsTransport %s: role=%s, action=%s", getId(), role, SSL_state_string_long(this->ssl));
        }
        else if ((where & SSL_CB_ALERT) != 0)
        {
            ...
            {
                hlogw("DtlsTransport %s: received DTLS %s alert: %s", getId(), alertType, SSL_alert_desc_string_long(ret));
            }
            else if ((where & SSL_CB_WRITE) != 0)
            {
                hlogd("DtlsTransport %s: sending DTLS %s alert: %s", getId(), alertType, SSL_alert_desc_string_long(ret));
            }
            else
            {
                hlogd("DtlsTransport %s: DTLS %s alert: %s", getId(), alertType, SSL_alert_desc_string_long(ret));
            }
        }
        else if ((where & SSL_CB_EXIT) != 0)
        {
            if (ret == 0){
                hlogd("DtlsTransport %s: role=%s, failed='%s'", getId(), role, SSL_state_string_long(this->ssl));
            }
            else if (ret < 0) {
                hlogd("DtlsTransport %s: role=%s, waiting='%s'", getId(), role, SSL_state_string_long(this->ssl));
            }
        }
        else if ((where & SSL_CB_HANDSHAKE_START) != 0)
        {
            hlogd("DtlsTransport %s: DTLS handshake start", getId());
        }
        else if ((where & SSL_CB_HANDSHAKE_DONE) != 0)
        {
            hlogd("DtlsTransport %s: DTLS handshake done", getId());
