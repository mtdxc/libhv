...
WebRtcTransport::Ptr WebRtcTransport::create(const char* type, const IceConfig *options) {
    if (strcmp(type, RTC_CLASS_ECHO) == 0)
        return Ptr(new WebRtcEcho(options, nullptr));
...
WebRtcTransport::Ptr WebRtcTransport::create(const char* type, IceAgent *agent) {
    if (strcmp(type, RTC_CLASS_ECHO) == 0)
        return Ptr(new WebRtcEcho(nullptr, agent));
    else if (strcmp(type, RTC_CLASS_PUSH) == 0)
