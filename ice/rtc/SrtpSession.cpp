...
DepLibSRTP &DepLibSRTP::Instance(){
    static std::shared_ptr<DepLibSRTP> instance(new DepLibSRTP());
    return *instance;
}
...
DepLibSRTP::DepLibSRTP() {
    ...
    if (DepLibSRTP::IsError(err)) {
        hlogw("srtp_init failed: %s", DepLibSRTP::GetErrorString(err));
        return;
    }
    ...
    err = srtp_install_event_handler([](srtp_event_data_t *data) {
        ...
        switch (data->event) {
            ...
            case event_key_hard_limit: hlogw("stream reached the hard key usage limit and has expired"); break;
            case event_packet_index_limit: hlogw("stream reached the hard packet limit (2^48 packets)"); break;
        }
    });

    if (DepLibSRTP::IsError(err)) {
        hlogw("srtp_install_event_handler() failed: %s", DepLibSRTP::GetErrorString(err));
    }
}
...
SrtpSession::SrtpSession(Type type, CryptoSuite cryptoSuite, uint8_t *key, size_t keyLen) {
    ...
    default:
        hlogw("unknown SRTP crypto suite");
        break;
    }
