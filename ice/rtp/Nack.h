...
public:
    ...
    NackContext(uint32_t ssrc = 0);
    uint32_t getIdentifier() const { return _ssrc; }
    void received(uint16_t seq, bool is_rtx = false);
    void setOnNack(onNack cb);
    uint64_t reSendNack();
    void UpdateRtt(uint32_t rtt);
    static void sendNack(const std::list<uint16_t>& seq, onNack cb);
private:
    void eraseFrontSeq();
...
private:
    ...
    // RTP seq value in the last nack packet
