...
static void setupHeader(RtcpHeader *rtcp, RtcpType type, size_t report_count, size_t total_bytes) {
    ...
    if (report_count > 0x1F) {
        throw std::invalid_argument("rtcp report_count最大赋值为31,当前为:" + std::to_string(report_count));
    }
    ...
    switch ((RtcpType)pt) {
        ...
        case RtcpType::RTCP_XR: {
            ...
            if (xr->bt == 4) {
                ...
                throw std::runtime_error("rtcp xr bt " + std::to_string((int)xr->bt) + " not support");
            }
            break;
        }
        default: throw std::runtime_error(std::string("未处理的rtcp包:") + rtcpTypeToStr((RtcpType)this->pt));
    }
}
...
vector<RtcpHeader *> RtcpHeader::loadFromBytes(char *data, size_t len) {
    ...
    while (remain > sizeof(RtcpHeader)) {
        ...
        if (remain < rtcp_len) {
            hlogw("skip rtcp packet, remain:%zu, rtcp_len:%zu", remain, rtcp_len);
            break;
        }
        try {
            ...
            hlogw("net2Host exception: %s, len:%zu", ex.what(), rtcp_len);
        }
        ptr += rtcp_len;
        remain -= rtcp_len;
    }
...
class BufferRtcp : public HBuf {
public:
    BufferRtcp(std::shared_ptr<RtcpHeader> rtcp) : HBuf(rtcp.get(), rtcp->getSize()) { _rtcp = std::move(rtcp); }
private:
    std::shared_ptr<RtcpHeader> _rtcp;
};

BufferPtr RtcpHeader::toBuffer(std::shared_ptr<RtcpHeader> rtcp) {
...
string RtcpSR::dumpString() const {
    ...
    printer << "ntp time:" << getNtpUnixStampMS() << "\r\n";
    printer << "rtpts:" << rtpts << "\r\n";
...
#define CHECK_MIN_SIZE(size, kMinSize)                                                                                 \
    if (size < kMinSize) {                                                                                             \
        throw std::out_of_range(                                                                                       \
            std::string(rtcpTypeToStr((RtcpType)pt)) + " size:" + std::to_string(size) + " < " + std::to_string(kMinSize));                   \
    }

#define CHECK_REPORT_COUNT(item_count)                                                                                 \
    /*修正个数，防止getItemList时内存越界*/                                                                                \
    if (report_count != item_count) {                                                                                  \
        hlogw("%s report_count dismatch change %d -> %d", rtcpTypeToStr((RtcpType)pt), (int)report_count, item_count); \
...
size_t RtcpFB::getFciSize() const {
    auto fci_len = (size_t)getSize() - getPaddingSize() - sizeof(RtcpFB);
...
void RtcpBye::net2Host(size_t size) {
    ...
    if (offset < size) {
        ...
        if (reason_len_ptr + 1 + *reason_len_ptr > (uint8_t *)this + size) {
            hlogw("invalid rtcp bye reason length");
...
////////////////////////////////////////////
std::shared_ptr<RtcpXRRRTR> RtcpXRRRTR::create(uint32_t ssrc, uint64_t ms) {
    auto real_size = sizeof(RtcpXRRRTR);
    auto bytes = alignSize(real_size);
    auto ptr = (RtcpXRRRTR *)new char[bytes];
    setupHeader(ptr, RtcpType::RTCP_XR, 0, bytes);
    setupPadding(ptr, bytes - real_size);
    ptr->bt = 4;
    ptr->block_length = htons(2);
    ptr->setNtpStamp(ms);
    return std::shared_ptr<RtcpXRRRTR>(ptr, [](RtcpXRRRTR *ptr) { delete[](char *) ptr; });
}
...
void RtcpXRRRTR::net2Host(size_t size) {
    ...
    if (size != sizeof(RtcpXRRRTR)) {
        throw std::invalid_argument(
            std::string("rtcp xr Receiver Reference Time Report Block must is ") + std::to_string(sizeof(RtcpXRRRTR))
