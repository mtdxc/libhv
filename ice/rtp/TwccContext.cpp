...
void TwccContext::onRtp(uint32_t ssrc, uint16_t twcc_ext_seq, uint64_t stamp_ms) {
    ...
    if (!result.second) {
        hlogw("recv same twcc ext seq: %d", twcc_ext_seq);
        return;
    }
...
int TwccContext::checkSeqStatus(uint16_t twcc_ext_seq) const {
    ...
    if (delta < -0xFF00) {
        ...
        // Loop
        hlogd("rtp twcc ext seq looped: %d -> %d", max, twcc_ext_seq);
    ...
    if (delta > 0xFF00) {
        ...
        hlogd("rtp twcc ext seq jumped after looped: %d -> %d", max, twcc_ext_seq);
    ...
    hlogd("rtp twcc ext seq jumped: %d -> %d", max, twcc_ext_seq);
    return (int) ExtSeqStatus::jumped;
}
