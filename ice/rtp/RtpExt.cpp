...
string RtpExt::dumpString() const {
    ...
    switch (_type) {
        ...
        case RtpExtType::framemarking : {
            auto mark = getFrameMarking();
            printer << "framemarking:" 
                << (mark->start ? 'S' : ' ') << (mark->end ? 'E' : ' ') 
                << (mark->independent ? 'I' : ' ') << (mark->discardable ? 'D' : ' ') << (mark->base ? 'B' : ' ') 
                << " tid=" << mark->tid << ",lid=" << mark->lid << ",pid=" << mark->tl0picidx;
            break;
        }
        default: {
...
const FrameMarking* RtpExt::getFrameMarking() const {
    CHECK(_type == RtpExtType::framemarking && size() >= 3);
    return (FrameMarking*)_data;
}
...
int RtpExtContext::changeRtpExtId(RtpHeader *header) {
    int ret = 0;
    ...
    for (auto &pr : ext_map) {
        ...
        if (it == _rtp_ext_type_to_id.end()) {
            // TraceL << "Sending rtp, ignoring unknown rtp ext:" << pr.second.dumpString();
            pr.second.clearExt();
            continue;
        }
        // 将ext id改成客户端sdp声明的类型
...
int RtpExtContext::parseRtpExtId(RtpPacket *rtp) {
    int ret = 0;
    string rid, repaired_rid;
    RtpHeader *header = rtp->getHeader();
    auto extMap = RtpExt::getExtValue(header);
    for (auto &pr : extMap) {
        auto it = _rtp_ext_id_to_type.find(pr.first);
        if (it == _rtp_ext_id_to_type.end()) {
            // TraceL << "Receiving rtp, ignoring unkonwn rtp ext, id=" << (int) pr.first;
            pr.second.clearExt();
            continue;
        }
        ret++;
        pr.second.setType(it->second);
        // 将ext的id设为type，方便后面统一处理
        pr.second.setExtId((uint8_t) it->second);
        rtp->extMap[(uint8_t)it->second] = pr.second;
        switch (it->second) {
