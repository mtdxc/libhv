...
public:
    ...
    Ticker(uint64_t min_ms = 0) {
        _created = _begin = gettick_ms();
        _min_ms = min_ms;
    }

    ~Ticker() {
        uint64_t tm = createdTime();
        if (_min_ms && tm > _min_ms) {
            hlogw("take time: %d ms, thread may be overloaded", tm);
        }
    }
