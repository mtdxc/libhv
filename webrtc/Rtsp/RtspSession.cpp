...
bool makeSockPair(std::shared_ptr<hv::SocketChannel> pair[2], const string& local_ip, bool re_use_port = true, bool is_udp = true) {
    ...
    static int port = s_min_port / 2;
    if (port * 2 > s_max_port)
        port = s_min_port / 2;
    hio_type_e type = is_udp ? HIO_TYPE_UDP : HIO_TYPE_TCP;
    hio_t* io = hio_create_socket(loop, local_ip.c_str(), port * 2, type);
    if(!io) return false;
    pair[0] = std::make_shared<hv::SocketChannel>(io);
    io = hio_create_socket(loop, local_ip.c_str(), port * 2 + 1, type);
    if(!io) {
        pair[0] = 0;
        return false;
    }
    pair[1] = std::make_shared<hv::SocketChannel>(io);
    return true;
}
...
void RtspSession::onAuthBasic(const string &realm, const string &auth_base64) {
    ...
    hv_base64_decode(auth_base64.data(), (int)auth_base64.size(), (uint8_t*)user_pwd_buf);
...
void RtspSession::handleReq_Setup(const Parser &parser) {
    ...
    case Rtsp::RTP_UDP: {
        char ipaddr[SOCKADDR_STRLEN];

        std::shared_ptr<hv::SocketChannel> pr[2];
        try {
        ...
        sockaddr_ip(addr, ipaddr, sizeof(ipaddr));
        sockaddr_u peerAddr;
        sockaddr_set_ipport(&peerAddr, ipaddr, ui16RtpPort);
...
void RtspSession::startListenPeerUdpData(int track_idx) {
    ...
    std::string peer_ip = sockaddr_ip((sockaddr_u*)hio_peeraddr(io_), buf, sizeof(buf));
    auto onUdpData = [weakSelf,peer_ip](const Buffer::Ptr &buf, struct sockaddr *peer_addr, int interleaved){
        auto strongSelf = weakSelf.lock();
        if (!strongSelf) {
            return false;
        }
        char temp[SOCKADDR_STRLEN] = { 0 };
        if (sockaddr_ip((sockaddr_u*)peer_addr, temp, sizeof(temp)) != peer_ip) {
        ...
        case Rtsp::RTP_UDP:{
            auto setEvent = [&](std::shared_ptr<hv::SocketChannel> sock, int interleaved){
                if (!sock) {
                    ...
                    sock->onread = [onUdpData, interleaved, sock](hv::Buffer* buf) {
                        struct sockaddr* pPeerAddr = hio_peeraddr(sock->io());
                        auto pBuf = BufferRaw::create();
                        pBuf->assign((char*)buf->data(), buf->size());
...
int RtspSession::getTrackIndexByTrackType(TrackType type) {
    ...
    throw SockException(Err_shutdown, std::string("no such track with type:") + getTrackString(type));
}
...
int RtspSession::getTrackIndexByControlUrl(const string &control_url) {
    ...
    throw SockException(Err_shutdown, std::string("no such track with control url:") + control_url);
}
...
void RtspSession::sendRtcpPacket(int track_idx, Buffer::Ptr ptr) {
    if (_rtp_type == Rtsp::RTP_TCP) {
