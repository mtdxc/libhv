...
std::shared_ptr<hv::SocketChannel> MediaSource::getOriginSock() const {
    auto listener = _listener.lock();
    if (!listener) {
        return nullptr;
    }
...
hv::EventLoopPtr MediaSource::getOwnerPoller() {
    hv::EventLoopPtr ret;
    auto listener = _listener.lock();
    if (listener) {
...
hv::EventLoopPtr MediaSourceEventInterceptor::getOwnerPoller(MediaSource &sender) {
    auto listener = _listener.lock();
    if (listener) {
