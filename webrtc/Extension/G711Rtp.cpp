...
G711RtpEncoder::G711RtpEncoder(CodecId codec, 
    uint32_t ssrc, uint32_t mtu_size, uint32_t sample_rate, uint8_t payload_type, uint8_t interleaved, uint32_t channels)
...
bool G711RtpEncoder::inputFrame(const Frame::Ptr &frame) {
    auto dur = _cache_frame->size() / (8 * _channels);
    auto next_pts = _cache_frame->pts() + dur;
    if (next_pts == 0) {
        _cache_frame->_pts = frame->pts();
    } else {
        if ((next_pts + 20) < frame->pts()) { // 丢包超过20ms
    ...
    auto ptr = _cache_frame->data();
    auto len = _cache_frame->size();
    auto remain_size = len;
    auto rtp_size = 160 * _channels; // 20 ms per rtp
    int n = 0;
    bool mark = false;
    while (remain_size >= rtp_size) {
        n++;
        stamp += 20;
    ...
    _cache_frame->_buffer.erase(0, n * rtp_size);
