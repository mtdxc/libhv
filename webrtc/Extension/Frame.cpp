...
static std::map<std::string, CodecId, hv::StringCaseLess> codec_map = {CODEC_MAP(XX)};
#undef XX
...
static std::map<std::string, TrackType, hv::StringCaseLess> track_str_map = {
        {"video",       TrackVideo},
        {"audio",       TrackAudio},
...
bool FrameMerger::willFlush(const Frame::Ptr &frame) const{
    if (_frame_cache.empty()) {
        ...
        LOGI("帧缓存过多:%d强制刷新", _frame_cache.size());
        return true;
    }
    if (!frame) {
        LOGI("flush with empty frame");
        return true;
